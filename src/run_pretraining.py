...
downcast_to_bf16 = 1  # Downcasts the flat_tensor to bf16 from FP32
...
def take_optimizer_step(args, optimizer, model, overflow_buf, global_step):
    ...
    if args.allreduce_post_accumulation and not args.use_habana:
        ...
        print(f"AMAR: allreduce_dtype = {allreduce_dtype}") # This branch is not executed for HPUs! Why?
        ...
        if  args.use_habana and args.allreduce_post_accumulation:
            #print(f"AMAR: CODE BRANCH TAKEN ===================================================")
            ...
            if downcast_to_bf16 == 1:
                ...
                temp = flat_tensor
                #print(f"AMAR:BEFORE DOWNCAST: temp.dtype = {temp.dtype}")
                temp = temp.to(mytype)
                #print(f"AMAR:AFTER DOWNCAST: temp.dtype = {temp.dtype}")
                torch.distributed.all_reduce(temp)
                temp = temp.to(torch.float32)
                #print(f"AMAR:AFTER UPCAST: temp.dtype = {temp.dtype}")
                flat_tensor = temp
            else:
    ...
    if args.do_train:
        ...
        while True:
            ...
            if torch.distributed.is_initialized() and get_world_size() > num_files:
                ...
                print(f"AMAR: f_start_id , get_world_size(), get_rank(), num_files = {f_start_id},{get_world_size()},{get_rank()}, {num_files} ")
